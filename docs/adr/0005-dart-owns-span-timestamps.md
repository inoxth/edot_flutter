...
- **The Android Agent may discard the timestamp Dart sent.** `ClockExporterGateManager` in `agent-sdk` 1.1.0 stamps every span created before its clock sync lands with a native elapsed-time reading, and rewrites that span's timestamps from it at export: `start = elapsed + offset`, `end = start + duration`. Duration survives exactly; the absolute times are the Agent's, taken when the channel call arrived rather than when Dart minted the span. Two spans minted from one Dart reading (ADR-0016's pair) therefore arrive a few nanoseconds apart. Nothing here needs changing - the Agent is correcting for a clock it trusts less than its own - but an assertion that Dart's exact timestamps reach the wire only holds once the gate has opened. Seam 2 tells the two cases apart by precision: Dart sends microseconds, so a surviving timestamp is a whole multiple of 1000ns.
