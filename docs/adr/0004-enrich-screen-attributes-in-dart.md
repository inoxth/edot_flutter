...
- **On the first frame of a tabbed host, the container's Screen Span is emitted and immediately overtaken by the initial tab's.** The route observer enters the host route before `EdotViewObserver` has mounted and claimed it, so a cold start shows the container name as a near-zero-duration transition that the initial tab's span overtakes. This is accepted rather than fixed: the deferral that makes a pop-back land on one span depends on the claim already existing, which it is not yet on first mount, and deferring the route observer's own enter would break its time-to-first-frame timing. Only the initial landing is affected; every later return is a single span.
